...
export const getV3Quote = async (
  ...
  const token0 = new Token(pool.token0.chainId, pool.token0.address, pool.token0.decimals, pool.token0.symbol, pool.token0.name);
  const token1 = new Token(pool.token1.chainId, pool.token1.address, pool.token1.decimals, pool.token1.symbol, pool.token1.name);
  ...
  if (tokenIn.address.toLowerCase() === token0.address.toLowerCase() && tokenOut.address.toLowerCase() === token1.address.toLowerCase()) {
    // tokenIn is token0, tokenOut is token1 - use price as-is
    preSwapPrice = token0Price;
  } else if (tokenIn.address.toLowerCase() === token1.address.toLowerCase() && tokenOut.address.toLowerCase() === token0.address.toLowerCase()) {
